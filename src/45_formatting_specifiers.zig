...
test "hex" {
    ...
    try expectEqualStrings("FFFFFFFE", try bufPrint(&b, "{X}", .{4294967294}));

    try expectEqualStrings("fffffffe", try bufPrint(&b, "{x}", .{4294967294}));

    try expectEqualStrings("0xAAAAAAAA", try bufPrint(&b, "0x{X}", .{2863311530}));

    try expectEqualStrings("5a696721", try bufPrint(&b, "{}", .{
        std.fmt.fmtSliceHexLower("Zig!"),
    }));

    try expectEqualStrings("4D61797572", try bufPrint(&b, "{}", .{
        std.fmt.fmtSliceHexUpper("Mayur"),
    }));
}
...
test "B Bi" {
    ...
    try expectEqualStrings("1.073741824GB", try bufPrint(&b, "{}", .{fmtIntSizeDec(1024 * 1024 * 1024)}));
    try expectEqualStrings("1GiB", try bufPrint(&b, "{}", .{fmtIntSizeBin(1024 * 1024 * 1024)}));
}
...
test "scientific" {
    ...
    try expectEqualStrings("3.14159e-1", try bufPrint(&b, "{e}", .{0.314159}));
}
